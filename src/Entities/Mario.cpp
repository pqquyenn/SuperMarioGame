...
AnimationFrame fullFrame(const sf::Texture *texture, int width, int height) {
...
PlayerAnimationSet
makeSmallAnimationSet(const MarioAnimationTextures &textures) {
  PlayerAnimationSet set;
  set.idle = staticClip(fullFrame(textures.smallIdle, SmallWidth, SmallHeight));
  set.running =
      AnimationClip{{fullFrame(textures.smallRun1, SmallWidth, SmallHeight),
                     fullFrame(textures.smallRun2, SmallWidth, SmallHeight),
                     fullFrame(textures.smallRun3, SmallWidth, SmallHeight)},
                    0.09f,
                    true};
  set.jumping =
      staticClip(fullFrame(textures.smallJump, SmallWidth, SmallHeight));
  set.sliding =
      staticClip(fullFrame(textures.smallSlide, SmallWidth, SmallHeight));
  set.crouching = set.idle;
  set.dead = AnimationClip{
      {fullFrame(textures.smallDeath ? textures.smallDeath : textures.smallIdle,
...
PlayerAnimationSet makeSuperAnimationSet(const MarioAnimationTextures &textures,
                                         const AnimationClip &deathClip) {
  PlayerAnimationSet set;
  set.idle = staticClip(fullFrame(textures.superIdle, SuperWidth, SuperHeight));
  set.running =
      AnimationClip{{fullFrame(textures.superRun1, SuperWidth, SuperHeight),
                     fullFrame(textures.superRun2, SuperWidth, SuperHeight),
                     fullFrame(textures.superRun3, SuperWidth, SuperHeight)},
                    0.09f,
                    true};
  set.jumping =
      staticClip(fullFrame(textures.superJump, SuperWidth, SuperHeight));
  set.sliding =
      staticClip(fullFrame(textures.superSlide, SuperWidth, SuperHeight));
...
makeIndividualMarioAnimationProfile(const MarioAnimationTextures &textures) {
...
bool MarioAnimationTextures::isValid() const {
  return hasSize(smallIdle, SmallWidth, SmallHeight) &&
...
std::string_view Mario::getCharacterType() const { return "Mario"; }
...
bool Mario::setAnimationTextures(const MarioAnimationTextures &textures) {
  ...
  setPlayerAnimationProfile(makeIndividualMarioAnimationProfile(textures));
  return true;
}
