...
// ============================================================
Mushroom::Mushroom(float x, float y) : Item(x, y) { size = {16.f, 16.f}; }
...
void Mushroom::update(float dt) {
  ...
  if (emerging) {
    ...
    // Di chuyển ngang & áp dụng trọng lực chuẩn Entity
    velocity.x = static_cast<float>(moveDirection) * moveSpeed;
    velocity.y += gravity * dt;

    // Tích phân vị trí: position += velocity * dt
...
bool Mushroom::isEmerging() const { return emerging; }

void Mushroom::reverseDirection() { moveDirection = -moveDirection; }

int Mushroom::getMoveDirection() const { return moveDirection; }
