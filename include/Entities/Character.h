...
protected:
  ...
  bool horizontalInputThisFrame{
      false}; // Prevents friction while movement input is active.
  bool jumpHeldThisFrame{
      false};            // True when jump input was received this frame.
  float shootTimer{0.f}; // Timer for shoot pose animation.
  float releaseDeceleration{
      1000.f}; // Braking rate selected by the last movement mode.
  float horizontalMovementScale{
      1.f}; // Ruleset multiplier; 1 keeps solo physics unchanged.
